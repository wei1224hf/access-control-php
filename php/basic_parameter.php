...
class basic_parameter {
	...
	public static function callFunction(){
		...
		else if($function =="typetree"){
			$t_return = self::typetree($_REQUEST['reference'],$_REQUEST['upcode'],$_REQUEST['hasall'],$_REQUEST['codelen'],$_REQUEST['executor']);
		}
    ...
    public static function typetree($reference,$upcode,$hasall,$codelen,$executor){
    	$conn = tools::getConn();
    	$sql = "select code,value from basic_parameter where reference = '".$reference."' and code like '".$upcode."%' and code <> '".$upcode."' order by code";
    	$res = tools::query($sql,$conn);
    	$data = array();
    	while ($temp = tools::fetch_assoc($res)){
    	...
    	tools::$list2Tree_basicLen = $codelen;
    	if($hasall){
    		array_unshift($data,array("code"=>"00","value"=>'所有','code_'=>"00"));
    		tools::$list2Tree_basicLen = 2;
    	}    	
